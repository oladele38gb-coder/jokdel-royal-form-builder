      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-2xs">
        ...
        <div className="flex items-center gap-2">
          ...
          {responses.length > 0 && onClearAllResponses && (
            <button
              onClick={() => {
                if (window.confirm('Are you sure you want to clear ALL responses? This will delete all client submissions and start completely afresh.')) {
                  onClearAllResponses();
                }
              }}
              className="inline-flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl text-xs font-semibold text-red-600 border border-red-200 bg-red-50 hover:bg-red-100 hover:border-red-300 transition-colors cursor-pointer"
              title="Delete all responses and start fresh"
            >
              <Trash2 className="w-3.5 h-3.5" />
              <span>Clear All ({responses.length})</span>
            </button>
          )}

          <button
            onClick={handleExportCSV}
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-semibold text-white transition-colors cursor-pointer"
          ...
          <div className="w-full max-w-2xl bg-white h-full shadow-2xl flex flex-col justify-between overflow-y-auto">
            ...
            <div className="p-5 border-b border-gray-100 flex items-start justify-between bg-white">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => onSelectResponseDetail(null)} className="p-2 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer">
